...
export default function ProfileBuyer() {
    ...
    return ( <
        ...
        h1 className = "mainh1"
        style = {
            { backgroundColor: '#f0a202', padding: '60px' } } > DEVELOPER 'S PROFILE:</h1> <
        div style = {
        ...
        p className = 'profiletext' > $ { user.hourlyRate }
        /hr</p >
        <
        ...
        href = { user.sampleProjectURL } > { user.sampleProjectURL } < /a> <
        h2 className = 'profiletitles' > Github URL < /h2> <
        a className = 'profiletext'
        href = { user.gitHub } > { user.gitHub } < /a> <
        h3 className = 'profiletitles' > Linkedin: < /h3>  <
        a className = 'profiletext'
        href = { user.linkedIn } > { user.linkedIn } < /a> <
        ...
        onClick = { handleShow }
        style = {
            { backgroundColor: '#4AB8B1', border: 'none', fontSize: '20px', padding: '12px 24px', marginTop: '15px' } } > Contact Developer < /Button> <
        /div> <
        Modal show = { show }
        onHide = { handleClose } >
        <
